...
def initTopo():
    inDeg = [len(d) for d in taskDep]
    adj = [[] for _ in range(NumberTasks)]
    for v in range(NumberTasks):
        for u, _ in taskDep[v]:
            adj[u].append(v)
    q = deque()
    for v in range(NumberTasks):
        if inDeg[v] == 0:
...
class Scheduler:
    ...
    def schedule(self):
        ...
        for i in range(NumberActualSlots):
            exeQue[i] = deque(sorted(exeQue[i], key = lambda t: topoOrder[t]))
